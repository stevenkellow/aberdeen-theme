...
function aberdeen_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'aberdeen' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'aberdeen' ),
...
function aberdeen_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $tags_list ) {
			printf( '<p class="tags-links"><i class="fa fa-tags fa-2x"></i>&nbsp;' . $tags_list . '</p>' ); // WPCS: XSS OK.
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( esc_html__( 'Leave a comment', 'aberdeen' ), esc_html__( '1 Comment', 'aberdeen' ), esc_html__( '% Comments', 'aberdeen' ) );
		echo '</span>';
	}
